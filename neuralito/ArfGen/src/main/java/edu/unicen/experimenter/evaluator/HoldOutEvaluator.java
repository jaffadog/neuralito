...
public class HoldOutEvaluator {
	...
	// final List<DataSet> dataSets, final File classifiersConfXml)
	// throws Exception {
	// // Read Classifiers configuration object from xml.
	// final XStream xstream = new XStream();
	// final ClassifiersConfiguration classifiersConf =
	// (ClassifiersConfiguration) xstream
	// .fromXML(new FileInputStream(classifiersConfXml));
	// final ClassifierConfiguration classifierConfiguration = classifiersConf
	// .getClassifiers().get(0);
	//
	// for (int i = 0; i < dataSets.size(); i++) {
	// final Classifier c = (Classifier) Utils
	// .forName(Classifier.class, classifierConfiguration
	// .getClassifierName(), Utils
	// .splitOptions(classifierConfiguration.getOptions()));
	//
	// // Convert datasets
	// System.out.println("Converting dataset with id = "
	// + dataSets.get(i).getId() + " to weka dataset.");
	// final Instances data = InstancesCreator
	// .generateTrainningData(dataSets.get(i));
	// // Train
	// System.out.println("Trainning classifier.");
	// c.buildClassifier(data);
	// // evaluate
	// System.out.println("Evaluating classifier.");
	// final Evaluation eval = new Evaluation(data);
	// final double[] evaluateModel = eval.evaluateModel(c, data);
	// eval.
	//
	// }
	// }
	 ...
	 * datasets, evaluates each one using 10 times 10-fold-CV. Results are saved
	 * to DB.
	 * 
	 */
