...
public class DataTimeFilter extends Filter {
	...
	public Vector<WaveData> executeFilter(Vector<?> dataSet) {
		...
		if (this.compareTime(this.minTime, this.maxTime) == 1) {
			...
			Calendar endOfDay = new GregorianCalendar(0, 0, 0, 23, 59);
			Calendar beginningOfDay = new GregorianCalendar(0, 0, 0, 0, 0);
			endOfDay.setTimeZone(TimeZone.getTimeZone("UTC"));
			beginningOfDay.setTimeZone(TimeZone.getTimeZone("UTC"));
			filters.add(new DataTimeFilter(this.minTime, endOfDay));
			filters.add(new DataTimeFilter(beginningOfDay, this.maxTime));
