...
public class WavesAndWindStrategy implements GenerationStrategy {
	...
	public WavesAndWindStrategy() {
		...
		this.description = 
			...
			"Tambien utiliza datos de direccion y velocidad del viento";
		
	}
	public String getShortDescription() {
		return shortDescription;
	}
	public WavesAndWindStrategy(String beach, String[]years, double ww3Y, double ww3X) {
		this();
		this.beach = beach;
		this.shortDescription = "strategy[WW3Last2DaysWIND].beach[" + beach + "].years " + Arrays.toString(years) + ".months[1-12].height[unrestriced].ww3.1[" + ww3Y + "," + ww3X+"]";
	}
	@Override
	...
	public String getDescription() {
		...
		return "Same as no buoy strtegy but using  previous day data and also wind data";
	}
	...
	private Vector<ArfData> mergeData(Vector<WaveWatchData> ww3DataSet, Vector<ObsData> obsDataSet){
		...
		for (int i = 1; i < ww3DataSet.size(); i++){
			...
			if (obsData != null){
				...
				data.put("ww3WindU", ww3Data.getWindU());
				data.put("ww3WindV", ww3Data.getWindV());
				...
				data.put("ww3WindUYesterday", ww3DataYesterday.getWindU());
				data.put("ww3WindVYesterday", ww3DataYesterday.getWindV());
