...
public class ExperimentDemo {
	...
	public static void main(final String[] args) throws Exception {
		...
		if (option.equals("crossvalidation")) {
			// final CrossValidationResultProducer cvrp = new
			...
			final MyResultProducer rp = new MyResultProducer();
			// final AveragingResultProducer rp = new AveragingResultProducer();
			...
			System.err.println("------------****************************");
			final RandomSplitResultProducer rsrp = new RandomSplitResultProducer();
		...
		final String[] options = Utils.splitOptions(option);
		final String classname = options[0];
		options[0] = "";
		...
		do {
			...
			if (option.length() > 0) {
				final File file = new File(option);
				if (!file.exists())
		...
		// final Instances result = new Instances(new BufferedReader(
		...
		// .index());
		// } else {
		// tester.multiResultsetFull(0, result.attribute(
