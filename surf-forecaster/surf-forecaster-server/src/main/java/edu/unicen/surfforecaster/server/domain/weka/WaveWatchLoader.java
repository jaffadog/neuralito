...
public class WaveWatchLoader {
	...
	public Collection getWWData(String[] years, double latitude,
		...
		for (int i = 0; i < years.length; i++) {
			String fileName = ".//files//ww3//ww3_" + years[i] + "Lat("
	 ...
	 */
	public static void loadDataFromGrib(String[] years, double latitude,
			double longitude) {
		for (int j = 0; j < years.length; j++) {
			...
			for (int i = 1; i <= 12; i++) {
				...
				// waveData.addAll(reader.getWaveWatchData(latitude,
				// longitude));
